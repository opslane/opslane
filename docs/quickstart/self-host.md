...
**What this proves:** ingestion, source-map resolution, identity, grouping, and the job queue, without any AI or GitHub account.
...
The `opslane_pk_...` value is the seed script's test ingest key, quoted at the top of `scripts/seed-e2e.sql`; real deployments mint their own. You should get HTTP `202` with an `error_group_id`. Within a few seconds the worker resolves the stack and Opslane groups the event into an issue. Check it:
...
The event was captured, its stack resolved, and it was grouped into a `new` issue. Opslane watches the issue but won't investigate a one-off that hasn't reached enough users yet, and with no AI credentials it couldn't investigate anyway. That takes Path 2.

Every issue Opslane *does* investigate ends in an explicit state: a ready-for-review fix PR backed by executed evidence (`pr_created`), an opt-in unverified draft awaiting repository CI (`pr_draft`), a posted root-cause analysis awaiting your go-ahead (`investigated`), or a stated reason a human is needed (`needs_human`).
