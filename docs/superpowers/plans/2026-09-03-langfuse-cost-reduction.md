...
- **Unsolved, and stated flat:** content capture. `shouldExportSpan: () => true` (`tracing.ts:191`) exports every span, no masking hook is configured, and the only redaction in the tracing path strips credentials from diagnostic log lines. Narration prompts carry session timelines with URLs, clicks and form submissions for 511 Jira tenant origins. Whether base64 screenshots also reach span attributes is unverified; reading `@arizeai/openinference-instrumentation-anthropic`'s multimodal handling would settle it. Nothing has exported since 2026-08-17 because ingestion refuses everything, so paying for Core turns this back on. **Settle it before the plan upgrade, not after.**
