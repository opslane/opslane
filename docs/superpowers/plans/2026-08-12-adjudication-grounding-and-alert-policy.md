...
2. `deriveOutcome` (`classify.ts:105-127`) demoted it to `needs_more_context` because two local candidate hypotheses were "unrejected". Both were fabricated: they named a `router/index.ts` `afterEach` hook and manifest keys that do not exist in the customer repo. The check requires each candidate's statement to appear verbatim (lowercased substring) in the free-prose `rejected` array, so even a genuine rejection in different words fails it.
