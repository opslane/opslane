...
- **Redact before emitting.** Exporter errors routinely quote request material, so every forwarded string passes through a redactor built from the resolved credentials plus credential-shaped patterns (`pk-lf-…`/`sk-lf-…`, `Bearer …`, `authorization: …`). Redaction runs *before* truncation, so a secret cannot survive as a fragment. The `authorization` pattern consumes the rest of the line rather than one token — a value-only match stops at the first space and leaves the credential in `authorization: Basic <base64>`. There is no minimum length on configured secrets: a floor would exempt exactly the credentials we were handed, and over-redacting a log line is far cheaper than leaking a key.
- **Redaction covers lifecycle errors too, not just diagnostics.** The init and shutdown catch handlers log SDK exceptions that can quote the configured keys, and those call sites sit outside the diag adapter. The redactor is therefore built before the `try` and retained for shutdown.
- Never throw. The whole body is wrapped in try/catch with an empty handler; a broken logger must not break the caller. Message and error stringification go through `safeErrorMessage`. The same applies on the lifecycle paths: `logger.warn` itself can throw (it JSON.stringifies fields unguarded), so shutdown and rollback emit through a guarded `safeWarn` — otherwise a logging failure would reject shutdown and skip `diag.disable()`.
...
- `trace_url` rejection logs `job_id` and a normalized error, exercised through `processJobInner` with the existing `index.test.ts` mocks. That mock factory must also export `safeErrorMessage`, or the new import resolves to `undefined` and the test dies before reaching the assertion.
- Test hygiene: `vi.resetModules()` does not clear OTel's global diag registration (it lives on a `globalThis` symbol), so the init tests call `diag.disable()` in `afterEach` to stop adapters leaking between tests.
