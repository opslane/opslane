...
| `PRODUCT_CONTEXT_MAX_TURNS` | no (route-scaled, max 80) | Overrides the route classifier's model/tool-step limit. Without an override it scales from 20 with the number of routes. |
| `PRODUCT_CONTEXT_BUDGET_USD` | no (route-scaled, max 3.00) | Overrides the route classifier's estimated model-spend ceiling. Without an override it scales from $0.50 with the number of routes. |
...
| `OPENAI_API_KEY` | no | OpenAI key for `text-embedding-3-small` ticket retrieval. Without it, new tickets have no embedding: the careful first look sees no similar tickets and creates duplicates, and the publish gate never asks the one-fix question. The worker warns at startup and `backfill-tickets` refuses to run without it unless passed `--allow-missing-embeddings`. |
...
| `FRICTION_FIRST_LOOK_MAX_TOKENS` | no (16384) | Output token ceiling for the careful first look. Invalid values, or values outside 1024 to 64000, fall back to the default. |
...
| `HEALTH_PORT` | no (8081) | Health endpoint port. The worker's `/health` returns `status: ok`, `stalled` (eligible work waiting, no jobs started in the last minute, and none running), or `unknown` (no queue sample has landed, or the last one is stale). It also returns `claims_per_minute`, a per-job-type `queue_depth`, and `dead_letters_24h` grouped by job type and failure class. `dead_letters_given_up` counts the dead letters the worker will never re-run again (three requeues spent); it never ages out, so a stuck incident stays visible until an operator acts. Queue and dead-letter counts are sampled once a minute, not every time a worker starts a job. |
