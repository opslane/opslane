...
| `job_dead_lettered` | worker; after `failJob` or the expired-lease reaper commits a terminal job failure | job ID, project ID, job type, failure class, attempts, requeues, scrubbed last error |
| `job_requeued` | worker; after `requeueDeadLetters` moves a dead letter back to pending | job ID, project ID, job type, failure class, requeues, trigger (`boot` or `interval`) |
| `job_given_up` | worker; when a job dead-letters again after its third requeue | job ID, project ID, job type, failure class, scrubbed last error |
...
For dead-letter events, `attempts` is the number of executions that actually ran; `requeues` is how many times the worker has re-run the dead letter.
