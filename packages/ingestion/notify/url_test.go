...
func TestBuildSessionURL(t *testing.T) {
	cases := []struct {
		...
		{"https with project", "https://app.example.com/base/?old=1", "project/1", "https://app.example.com/base/sessions/session%2F1?project_id=project%2F1&t=4200"},
		{"https without project", "https://app.example.com/base/?old=1", "", "https://app.example.com/base/sessions/session%2F1?t=4200"},
		{"empty", "", "project/1", ""},
		{"empty session", "https://app.example.com", "project/1", ""},
		{"loopback", "http://127.0.0.1:3000", "project/1", ""},
		{"credentials", "https://user:pass@app.example.com", "project/1", ""},
		{"wrong scheme", "javascript:alert(1)", "project/1", ""},
		{"fragment", "https://app.example.com/#fragment", "project/1", ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			sessionID := "session/1"
			...
			if got := BuildSessionURL(tc.base, sessionID, tc.projectID, 4200); got != tc.want {
				t.Fatalf("got %q want %q", got, tc.want)
			}
		})
	}
