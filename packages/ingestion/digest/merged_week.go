...
func mergedThisWeek(ctx context.Context, tx pgx.Tx, projectID string, at time.Time) ([]notify.DigestPRMerged, error) {
 ...
 SELECT DISTINCT ON(a.id) g.id,g.title,coalesce(nullif(e.pr_url,''),a.pr_url,'') AS pr_url,coalesce(e.pr_number,a.pr_number,0) AS pr_number,e.occurred_at
 ...
 SELECT DISTINCT ON(g.id,coalesce(o.github_repo,p.github_repo),o.pr_number) g.id,g.title,coalesce('https://github.com/'||coalesce(o.github_repo,p.github_repo)||'/pull/'||o.pr_number,CASE WHEN g.pr_number=o.pr_number THEN nullif(g.pr_url,'') END,'') AS pr_url,o.pr_number,o.occurred_at
 ...
 ORDER BY g.id,coalesce(o.github_repo,p.github_repo),o.pr_number,o.occurred_at DESC,o.id DESC
