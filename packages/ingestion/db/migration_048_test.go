...
func TestMigration048JobEventAnchor(t *testing.T) {
	admin := testPool(t)
	psql := findPsql(t)
	...
	if err := applyMigration(t, psql, dsn, "migrations/048_job_event_anchor.sql"); err != nil {
		t.Fatalf("migration 048 is not idempotent on reapply: %v", err)
	}
}
