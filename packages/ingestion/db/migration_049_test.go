...
func TestMigration049ActionScope(t *testing.T) {
	admin := testPool(t)
	psql := findPsql(t)
	pool, dsn := disposableDB(t, admin)
	for _, file := range migrationFiles(t) {
		if filepath.Base(file) == "049_action_scope.sql" {
			break
		}
	...
	if err := pool.QueryRow(ctx, `INSERT INTO orgs (name) VALUES ('migration-049') RETURNING id`).Scan(&orgID); err != nil {
		t.Fatal(err)
	}
	...
	if err := applyMigration(t, psql, dsn, "migrations/049_action_scope.sql"); err != nil {
		t.Fatal(err)
	}
	...
	if err := applyMigration(t, psql, dsn, "migrations/049_action_scope.sql"); err != nil {
		t.Fatalf("migration 049 is not idempotent on reapply: %v", err)
	}
}
