...
func cleanupTenant(t *testing.T, pool *pgxpool.Pool, orgID string) {
	...
	queries := []string{
		...
		`DELETE FROM sourcemap_batch_files WHERE project_id IN (SELECT id FROM projects WHERE org_id = $1)`,
		`DELETE FROM sourcemap_files WHERE project_id IN (SELECT id FROM projects WHERE org_id = $1)`,
		`DELETE FROM sourcemap_batches WHERE project_id IN (SELECT id FROM projects WHERE org_id = $1)`,
