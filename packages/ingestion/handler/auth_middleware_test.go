...
func TestUpdateProjectEndpoint_FrictionAutonomy(t *testing.T) {
	...
	if response := patch(`{"friction_autonomy":"auto_fix_ux"}`); response.Code != http.StatusOK {
		t.Fatalf("deprecated autonomy alias status = %d, want 200: %s", response.Code, response.Body.String())
	} else if !strings.Contains(response.Body.String(), `"friction_autonomy":"auto_fix"`) {
		t.Fatalf("deprecated autonomy alias was not stored as auto_fix: %s", response.Body.String())
	}
