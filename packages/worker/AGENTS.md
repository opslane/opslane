...
- `FRICTION_CONFIRM_DAILY_CAP` reserves one project/UTC-day unit per unstaged recording check in PostgreSQL; do not reuse narrative session budget stamps. The unit covers the read and its single retry. A resumed batch skips staged recordings. `FRICTION_CONFIRM_MAX_CONCURRENT=0` pauses confirmation and reconciliation.
- Confirmation answers in free-text JSON that code validates, with the model's default thinking and a 16,000-token limit; the one-fix gate uses the same settings. An invalid answer is retried once with the same request; a refusal is never retried. An answer still invalid after that retry, or a refusal, is staged `unavailable` and logged with the broken `rule` and the model's `stop_reason`; unavailable checks never count toward the publish bar; a recording is retried after 1, 6 and 24 hours, and its fourth unavailable attempt is permanent. Unavailable notes name the reason (recording missing, replay crashed, capture failed, no frames, invalid answer); count them with `SELECT note,count(*) FROM friction_check_attempts WHERE outcome='unavailable' GROUP BY note`.
- A one-fix gate answer still invalid after one retry never fails a confirmation batch: the batch finalizes and publication waits for reconciliation. Each twice-invalid gate answer counts toward the fold retry limit (`FOLD_RETRY_LIMIT`, 3); once it is spent the ticket publishes without the gate.
