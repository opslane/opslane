import { DEBUG_ID_TRAILER, MapTooLargeError, stampCodeAndMap, type StampResult, stripMapSuffix, stripSourceMappingURLDirectives, unstamp, preludeForFormat, assetBytes, normalizePath, isAbsolutePath, directoryOf, canonicalFilesystemPath, formatBytes } from '../src/build/stamp.js';
...
export function opslaneVitePlugin(
  ...
  const stampOne = async (
    ...
    prelude: string,
  ): Promise<StampResult> => stampCodeAndMap({
    code, mapSource: mapAsset.source, mapFileName: mapAsset.fileName,
    format: undefined, prelude, projectRoot, outDir, maxMapBytes,
  });
